...
async function fetchLoads() {
  const { data: loads, error } = await supabase
    .from("loads")
    .select(
      "*, customer:customers(name), assignments:load_assignments(*, driver:drivers(name), truck:vehicles(truck_number))",
    )
...
function renderKanbanView(loads) {
  ...
  loads.forEach((load) => {
    ...
    `;
    if (kanbanColumns[load.status])
      kanbanColumns[load.status].appendChild(card);
  });
}
