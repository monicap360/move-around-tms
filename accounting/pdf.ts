...
export async function generateInvoicePDF(invoice: Invoice): Promise<Uint8Array> {
  ...
  page.drawText('INVOICE', { x: 50, y: 800, size: 24, font, color: rgb(0,0,0) });
  page.drawText(`Invoice ID: ${invoice.id}`, { x: 50, y: 770, size: 12, font });
  page.drawText(`Issued: ${invoice.issued_at}`, { x: 50, y: 755, size: 12, font });
  page.drawText(`Due: ${invoice.due_at}`, { x: 50, y: 740, size: 12, font });
  page.drawText(`Status: ${invoice.status}`, { x: 50, y: 725, size: 12, font });
  page.drawText(`Customer: ${invoice.user_id}`, { x: 50, y: 710, size: 12, font });
  page.drawText(`Description: ${invoice.description}`, { x: 50, y: 695, size: 12, font });
  ...
  page.drawText(`Total: $${invoice.amount.toFixed(2)} ${invoice.currency}`, { x: 50, y: y-10, size: 14, font });

  if (invoice.zelle_receipt_url) {
    page.drawText(`Zelle Receipt: ${invoice.zelle_receipt_url}`, { x: 50, y: y-40, size: 10, font, color: rgb(0.2,0.2,0.2) });
  }
