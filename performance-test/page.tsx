...
export default function PerformanceTestPage() {
  ...
  const runPerformanceTests = async () => {
    ...
    const listByFolderTest = await measureTime(
    ...
    const multipleFoldersTest = await measureTime(
      "Multiple Folder Searches",
      async () => {
        const folders = [
          "user_uploads/",
          "documents/",
  ...
  const compareWithBaseline = () => {
    if (baseline.length === 0) {
      console.log(
        "No baseline saved yet. Run tests and save as baseline first.",
      );
      return;
    }
    ...
    return results
      .map((current, index) => {
        ...
        const improvement =
          ((baselineTest.duration - current.duration) / baselineTest.duration) *
          100;
        return {
          test: current.test,
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <p className="text-sm text-gray-600">
            Use this to measure before/after performance of database
            optimization
          </p>
        </CardHeader>
                  ...
                  {comparison.map((comp, index) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div
                          className={`text-sm font-bold ${comp.improvement > 0 ? "text-green-600" : comp.improvement < -10 ? "text-red-600" : "text-gray-600"}`}
                        >
                          {comp.status}{" "}
                          {comp.improvement > 0
                            ? `(${comp.improvement}% faster)`
                            : comp.improvement < -10
                              ? `(${Math.abs(comp.improvement)}% slower)`
                              : ""}
                        </div>
                      </div>
                    </div>
