...
export async function GET(
  ...
  if (urlErr || !signed) {
    return NextResponse.json(
      { error: urlErr?.message || "Failed to create signed URL" },
      { status: 500 },
    );
  }
...
export async function POST(
  ...
  if (!signature_base64) {
    return NextResponse.json(
      { error: "Missing signature_base64" },
      { status: 400 },
    );
  }
  ...
  if (download.error || !download.data) {
    return NextResponse.json(
      { error: download.error?.message || "Failed to download PDF" },
      { status: 500 },
    );
  }
