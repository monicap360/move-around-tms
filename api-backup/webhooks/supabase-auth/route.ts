...
export async function POST(req: NextRequest) {
  ...
  if (!isConfirmedEvent) {
    return NextResponse.json({ ok: true, skipped: true, reason: 'Not a confirmation event' })
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ ok: false, message: 'Failed to assign role' }, { status: 500 })
  }

  return NextResponse.json({ ok: true, assigned: { user_id: userId, role: defaultRole, company } })
}
