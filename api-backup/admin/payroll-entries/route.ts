...
export async function POST(req: NextRequest) {
  ...
  const { employee_id, pay_period_start, pay_period_end, total_hours, hourly_rate, percentage_rate, load_revenue, deductions, pay_type } = body
  const { data, error } = await supabaseAdmin
    .from('payroll_entries')
    .insert([{ employee_id, pay_period_start, pay_period_end, total_hours, hourly_rate, percentage_rate, load_revenue, deductions, pay_type }])
    .select('*, employees(full_name)')
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
