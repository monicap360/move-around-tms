...
export async function GET(req: NextRequest) {
  ...
  const until = new Date(Date.now() + upcomingDays*24*60*60*1000).toISOString().slice(0,10)
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('compliance_items').insert([body]).select('*').single()
  if (error) return NextResponse.json({ ok:false, error: error.message }, { status: 500 })
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('compliance_items').update(updates).eq('id', id).select('*').single()
  if (error) return NextResponse.json({ ok:false, error: error.message }, { status: 500 })
  return NextResponse.json({ ok:true, item: data })
}
