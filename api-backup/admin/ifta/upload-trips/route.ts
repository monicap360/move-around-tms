...
export async function POST(req: NextRequest) {
  ...
  for (const r of rows) {
    ...
    if (!tripId) {
      const { data: t, error } = await supabaseAdmin.from('eld_trips').insert([{ truck_id: r.truck_id || null, start_time: r.start_time, end_time: r.end_time, total_miles: Number(r.miles||0) }]).select('id').single()
      if (error) return NextResponse.json({ ok:false, error: error.message }, { status: 500 })
      tripId = t.id
    }
    const { error: segErr } = await supabaseAdmin.from('eld_trip_segments').insert([{ trip_id: tripId, jurisdiction_code: r.jurisdiction_code, miles: Number(r.miles||0) }])
