...
export async function GET(req: NextRequest) {
  ...
  if (!(quarter >= 1 && quarter <= 4))
    return NextResponse.json(
      { ok: false, error: "invalid_quarter" },
      { status: 400 },
    );
  ...
  if (tripsErr)
    return NextResponse.json(
      { ok: false, error: tripsErr.message },
      { status: 500 },
    );
  ...
  if (fuelErr)
    return NextResponse.json(
      { ok: false, error: fuelErr.message },
      { status: 500 },
    );
  ...
  if (rateErr)
    return NextResponse.json(
      { ok: false, error: rateErr.message },
      { status: 500 },
    );
  ...
  for (const code of jurisCodes) {
    ...
    totals.miles += miles;
    totals.gallons_purchased += gallonsPurchased;
    totals.taxable_gallons += taxableGallons;
    totals.tax_due += taxDue;
    totals.tax_paid += taxPaid;
    totals.net_tax += netTax;
  }
