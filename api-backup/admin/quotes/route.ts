...
export async function POST(request: NextRequest) {
  ...
  if (!company || !contact_email || !billing_type || rate == null) {
    return NextResponse.json(
      { error: 'Missing required fields: company, contact_email, billing_type, rate' },
      { status: 400 }
    );
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (updates.rate !== undefined || updates.pay_rate !== undefined) {
    ...
    const newRate = updates.rate !== undefined ? updates.rate : existing?.rate || 0;
    const newPayRate = updates.pay_rate !== undefined ? updates.pay_rate : existing?.pay_rate || 0;
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await supabaseAdmin.from('aggregate_quotes').delete().eq('id', id);
