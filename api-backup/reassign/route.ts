...
export async function POST(request: Request) {
  try {
    ...
    const neededType = requiredTruckType || (driverName ? undefined : undefined)
    ...
    const candidate = MOCK_TRUCKS.find(t =>
      t.status === 'Ready' && (neededType ? t.truck_type === neededType : (quals.length ? quals.includes(t.truck_type) : true))
    )

    if (!candidate) {
      return NextResponse.json({ ok: false, message: 'No available qualified truck found (mock).' }, { status: 200 })
    }

    return NextResponse.json({ ok: true, suggestedTruck: candidate }, { status: 200 })
  } catch (err: any) {
