...
export async function getBrandingConfig(domain: string): Promise<BrandingConfig> {
  ...
  const { data, error } = await supabase
    .from("branding")
    .select("companyName,logoUrl,faviconUrl,primaryColor,accentColor,backgroundColor")
    .eq("domain", domain)
    .single();
  if (error || !data) {
    // fallback to default
