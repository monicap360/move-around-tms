...
if (typeof window !== "undefined") {
  if (!PUBLIC_URL || !/^https?:\/\//.test(PUBLIC_URL)) {
    ...
    console.error(
      "Supabase client: NEXT_PUBLIC_SUPABASE_URL is missing or invalid. Set it to your project URL (https://xxxx.supabase.co).",
    );
  }
  if (!PUBLIC_ANON_KEY) {
    // eslint-disable-next-line no-console
    console.error(
      "Supabase client: NEXT_PUBLIC_SUPABASE_ANON_KEY is missing. Set it to your project anon key.",
    );
  }
}
