  ...
  const runTests = async () => {
    ...
    try {
      ...
      testResults.push({
        ...
        result: response.ok ? "✅ Success" : `❌ HTTP ${response.status}`,
        details: response.ok
          ? `Server authenticated user, found ${data.files?.length || 0} files`
          : data,
      });
    } catch (err) {
      testResults.push({
    ...
    try {
      const testFile = new File(
        ["test content from server auth"],
        "server-auth-test.txt",
        { type: "text/plain" },
      );
    ...
    try {
      const testFile = new File(["test content"], "debug-test.txt", {
        type: "text/plain",
      });
    ...
    try {
      ...
      testResults.push({
        ...
        result: response.ok ? "✅ Success" : `❌ HTTP ${response.status}`,
        details: response.ok
          ? `Admin status: ${data.isAdmin ? "Admin" : "Regular User"}, Email: ${data.email || "None"}`
          : data,
      });
    } catch (err) {
      testResults.push({
    ...
    try {
      ...
      testResults.push({
        ...
        result: response.ok ? "✅ Success" : `❌ HTTP ${response.status}`,
        details: response.ok
          ? `Found ${data.admins?.length || 0} admins`
          : data,
      });
    } catch (err) {
      testResults.push({
    ...
    try {
      ...
      const testFile = new File([blob], "avatar-test.png", {
        type: "image/png",
      });
      const formData = new FormData();
      ...
      testResults.push({
        ...
        result: response.ok ? "✅ Success" : `❌ HTTP ${response.status}`,
        details: response.ok
          ? `Avatar uploaded: ${data.avatar_url ? "URL generated" : "No URL"}`
          : data,
      });
    } catch (err) {
      testResults.push({
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            ...
            <Button
              ...
              onClick={() =>
                window.open(
                  "https://movearoundtms.com/api/debug-production",
                  "_blank",
                )
              }
            >
