...
export default function TicketsPage() {
  ...
  useEffect(() => {
    ...
    const channel = realtimeClient.channel(`ticket:${selectedTicket.id}:ocr`, {
      config: { private: true },
    });
  ...
  // Handlers
  const updateOCRUI = useCallback(
    (data: any, ticket: AggregateTicket | null) => {
      setOcrFields(data.fields);
      setOcrStatus("completed");
      setTickets((prev: AggregateTicket[]) =>
        prev.map((t) =>
          t.id === ticket?.id ? { ...t, status: "ocr_completed" } : t,
        ),
      );
    },
    [],
    ...
    try {
      ...
      if (newTicket.image) {
        ...
        const { data: ocr } = await supabase.functions.invoke("vision-ocr", {
          body: { image_url: url, organization_id: organizationId, ticket_id },
        });
        if (ocr) {
          setNewTicket((nt: any) => ({
            ...nt,
  ...
  async function approveTicket() {
    ...
    try {
      ...
      await supabase
        .from("aggregate_tickets")
        .update({ ...updatedFields, status: "approved" })
        .eq("id", selectedTicket.id);
      await supabase.rpc("calculate_driver_pay", {
        ticket_id: selectedTicket.id,
      });
      await supabase.rpc("auto_advance_load", { ticket_id: selectedTicket.id });
      if (realtimeClient) {
        const channel = realtimeClient.channel(
          `ticket:${selectedTicket.id}:ocr`,
          { config: { private: true } },
        );
        channel.send({
          type: "broadcast",
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <input
                  ...
                  accept="image/*,.pdf"
                  onChange={(e) =>
                    setNewTicket((nt: typeof newTicket) => ({
                      ...nt,
