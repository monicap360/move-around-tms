  ...
  useEffect(() => {
    async function load() {
      const [k, h, l, s, d, a] = await Promise.all([
        ...
        fetch(`/api/company/${company}/analytics/heatmap`).then((r) =>
          r.json(),
        ),
        fetch(`/api/company/${company}/analytics/lanes`).then((r) => r.json()),
        fetch(`/api/company/${company}/analytics/scorecards`).then((r) =>
          r.json(),
        ),
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <KpiCard
          ...
          label="Profit per Lane"
          value={kpis?.profit_per_lane}
        />
        <KpiCard
        ...
        <KpiCard
          ...
          label="Avg Payroll Diff"
          value={kpis?.payroll_diff}
        />
      </div>
      ...
      <Card>
        <h2 className="font-semibold text-xl mb-4 flex items-center gap-2">
          <MapPin className="w-5 h-5" /> Cycle Time Heatmap
        </h2>
      ...
      <Card>
        <h2 className="font-semibold text-xl mb-4 flex items-center gap-2">
          <BarChart2 className="w-5 h-5" /> Profitability per Lane
        </h2>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100 border-b">
      ...
      <Card>
        <h2 className="font-semibold text-xl mb-4 flex items-center gap-2">
          <UserCheck className="w-5 h-5" /> Driver Scorecards
        </h2>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100 border-b">
      ...
      <Card>
        <h2 className="font-semibold text-xl mb-4 flex items-center gap-2">
          <Clock className="w-5 h-5" /> Payroll Diff (Since Last Week)
        </h2>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100 border-b">
            ...
            <tbody>
              {diffs.map((row, i) => (
                <tr key={i} className="border-b">
                  <td className="p-3 font-semibold">{row.driver}</td>
                  <td
                    className={`p-3 font-bold ${row.change > 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {row.change > 0 ? "+" : ""}
                    {row.change}
                  </td>
      ...
      <Card>
        <h2 className="font-semibold text-xl mb-4 flex items-center gap-2">
          <AlertTriangle className="w-5 h-5" /> Audit Log
        </h2>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100 border-b">
