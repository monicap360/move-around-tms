...
export default function AdvisorPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/advisor/leads?search=${encodeURIComponent(search)}&status=${status}&date=${date}&page=${page}`)
      .then(res => res.json())
      .then(data => {
        setLeads(data.leads || []);
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-gray-100 rounded shadow p-4 text-center">
          ...
          <div className="text-2xl font-bold text-gray-700">{summary.cold ?? '-'}</div>
        </div>
        <div className="bg-blue-50 rounded shadow p-4 text-center">
          <div className="text-gray-500 text-sm">Leads This Week</div>
          <div className="text-2xl font-bold text-blue-600">{summary.thisWeek ?? '-'}</div>
        </div>
      </div>
      {/* Search + Filter Bar */}
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={lead.id + '-' + lead.source}>
                ...
                <td className="p-3">
                  <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${statusColor(lead.status)}`}>{lead.status || 'New'}</span>
                </td>
                <td className="p-3">{lead.source}</td>
                <td className="p-3">{lead.created_at ? new Date(lead.created_at).toLocaleDateString() : '-'}</td>
                <td className="p-3">
                  {lead.override_status === 'trial' && (
                    <span className="inline-block bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-semibold">Trial</span>
                  )}
                  {lead.override_status === 'override' && (
                    <span className="inline-block bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs font-semibold">Override</span>
                  )}
                  {lead.override_status === 'permanent' && (
                    <span className="inline-block bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-semibold">Permanent</span>
                  )}
                  {!lead.override_status && (
                    <span className="inline-block bg-gray-100 text-gray-500 px-2 py-1 rounded text-xs">-</span>
                  )}
                </td>
              </tr>
            ))}
