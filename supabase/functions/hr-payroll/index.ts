...
function calcPay(entry: any, emp: any) {
  ...
  switch (emp.pay_type) {
    ...
    case "percentage":
      g =
        Number(entry.load_revenue || 0) *
        (Number(emp.percentage_rate || 0) / 100);
      break;
    case "salary":
...
async function createPayStubPDF(emp: any, pay: any) {
  ...
  const draw = (y: number, text: string) =>
    page.drawText(text, { x: 50, y, size: 12, font, color: rgb(0, 0, 0) });

  let y = 740;
  draw(y, "MoveAround TMS – Pay Stub");
  y -= 25;
  draw(y, `Employee: ${emp.full_name}`);
  y -= 18;
  draw(y, `Role: ${emp.role_type ?? "-"}`);
  y -= 18;
  draw(y, `Period: ${pay.pay_period_start} → ${pay.pay_period_end}`);
  y -= 18;
  draw(y, `Pay Type: ${emp.pay_type}`);
  y -= 18;
  draw(y, `Gross Pay: $${Number(pay.gross_pay ?? 0).toFixed(2)}`);
  y -= 18;
  draw(y, `Deductions: $${Number(pay.deductions ?? 0).toFixed(2)}`);
  y -= 18;
  draw(y, `Net Pay: $${Number(pay.net_pay ?? 0).toFixed(2)}`);
  y -= 18;
...
async function createContractPDF(emp: any, terms: string) {
  ...
  const draw = (y: number, t: string) =>
    page.drawText(t, { x: 50, y, size: 12, font });
  let y = 740;

  draw(y, "Independent Contract / Employment Agreement");
  y -= 30;
  draw(y, `Employee: ${emp.full_name}`);
  y -= 20;
  draw(y, `Role: ${emp.role_type ?? "-"}`);
...
serve(async (req) => {
  ...
  try {
    ...
    if (
      method === "POST" &&
      (path.endsWith("/hr-payroll") || path.endsWith("/hr-payroll/"))
    ) {
      try {
