...
serve(async (req) => {
  ...
  try {
    ...
    if (!fileUrl && !imageBase64) return json({ error: 'Missing file_url/imageUrl or imageBase64' }, 400)

    // OCR via Google Vision
    const [result] = await visionClient.textDetection(fileUrl || { content: imageBase64 })
    ...
    if (kind === 'hr' || (!kind && shouldTreatAsHr(rawText, body))) {
      const out = await handleHrDoc({ rawText, fileUrl, body, supabase, result })
      return json(out)
    } else {
      const out = await handleTicket({ rawText, fileUrl, body, supabase, result })
      return json(out)
    }
  } catch (error: any) {
...
async function handleTicket({ rawText, fileUrl, body, supabase, result }: any) {
  ...
  for (const p of (partners || []) as Partner[]) {
    ...
    if (p.regex_patterns?.company_hint) {
      ...
      if (regex.test(rawText)) { matchedPartner = p; break }
    }
  }
...
function extractTicketData(text: string, partner: Partner | null): any {
  ...
  const driverRegex = /(?:driver|operator|hauler)\s*:?\s*([a-z]+(?:\s+[a-z]+)*)/i
...
async function matchDriverFromOcr(
  ...
  for (const d of drivers || []) {
    ...
    for (const p of parts) if (p.length > 3 && lower.includes(p)) return { id: d.id, name: d.name, confidence: 60 }
  }
