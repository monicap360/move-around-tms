      ...
      return prev.map((l) => {
        if (l.status === "unassigned" || l.status === "pending") {
          ...
          for (let d of availableDrivers) {
            if (
              !l.required_endorsement ||
              (d.endorsements &&
                d.endorsements.includes(l.required_endorsement))
            ) {
              assignedDriver = d;
              break;
            }
          ...
          if (!assignedDriver && availableDrivers.length > 0) {
            assignedDriver =
              availableDrivers[driverIdx % availableDrivers.length];
            driverIdx++;
          }
          if (assignedDriver) {
            return {
          ...
          >
            <div className="font-bold mb-2 flex items-center gap-2">
              ...
              {col.key === "unassigned" && (
                <Button
                  size="sm"
                  onClick={() => setAssignModal({ open: true, load: null })}
                >
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
        <h2 className="font-bold text-lg mb-4">
          {load ? "Edit Assignment" : "Create Load"}
        </h2>
