...
export function calcTrends(
  ...
  for (const e of events) {
    ...
    if (e.metricPath.startsWith('compliance') && e.severity === 'critical') b.complianceFailures++
    // By severity
    if (!b.bySeverity[e.severity]) {
      b.bySeverity[e.severity] = { alerts: 0, acknowledgements: 0, escalations: 0, complianceFailures: 0 }
    }
    ...
    if (e.metricPath.startsWith('compliance') && e.severity === 'critical') b.bySeverity[e.severity].complianceFailures++
  }
