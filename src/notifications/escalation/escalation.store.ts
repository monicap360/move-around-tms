...
export async function setEscalationRules(organizationId: string, updates: EscalationRule[]): Promise<void> {
  for (const update of updates) {
    const { error } = await supabase
      .from('escalation_rules')
      .upsert([{ ...update, organizationId }], { onConflict: ['organizationId', 'severity'] })
    if (error) throw error
  }
}
