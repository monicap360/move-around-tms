    ...
    <div className="p-8 space-y-8 bg-[#0e1116] min-h-screen text-white">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <Wrench className="w-8 h-8 text-yellow-400" /> AI Maintenance Predictor
        </h1>
        <button
          ...
          {predictions.map((truck: any, i: number) => (
            <Card key={i} className="bg-white/5 border border-white/10 p-6 rounded-2xl shadow-xl backdrop-blur-lg hover:bg-white/10 transition text-white">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-bold text-lg">Truck #{truck.truck_number}</span>
              </div>
              <div className="flex items-center gap-2 mb-1">
                <Gauge className="w-5 h-5 text-yellow-300" />
                <span className="text-sm">Health Score:</span>
                <span className="font-bold text-xl text-yellow-200">{truck.health_score}</span>
              </div>
              <div className="flex items-center gap-2 mb-1">
                <AlertTriangle className="w-5 h-5 text-red-400" />
                <span className="text-sm">Breakdown Risk:</span>
                <span className="font-bold text-xl text-red-300">{truck.breakdown_risk}%</span>
              </div>
              <div className="text-gray-300 text-sm mt-2">
                <span className="font-semibold">Next Predicted Issue:</span> {truck.next_issue}
              </div>
