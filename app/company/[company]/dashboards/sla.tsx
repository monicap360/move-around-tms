      ...
      {!metrics ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <SLAStat label="MTTA (mean)" value={formatSeconds(metrics.mttaMean)} />
            <SLAStat label="MTTA (median)" value={formatSeconds(metrics.mttaMedian)} />
            <SLAStat label="MTTA (95th)" value={formatSeconds(metrics.mtta95th)} />
            <SLAStat label="Acknowledgement Rate" value={metrics.acknowledgedRate ? (metrics.acknowledgedRate * 100).toFixed(1) + '%': '-'} />
            <SLAStat label="Escalation Rate" value={metrics.escalationRate ? (metrics.escalationRate * 100).toFixed(1) + '%': '-'} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {['critical', 'warn', 'info'].map(sev => (
              <div key={sev} className="bg-white rounded shadow p-4">
                ...
                <div>MTTA (mean): {formatSeconds(metrics.bySeverity[sev]?.mttaMean)}</div>
                <div>MTTA (median): {formatSeconds(metrics.bySeverity[sev]?.mttaMedian)}</div>
                <div>Ack Rate: {metrics.bySeverity[sev]?.acknowledgedRate ? (metrics.bySeverity[sev].acknowledgedRate * 100).toFixed(1) + '%' : '-'}</div>
                <div>Escalation Rate: {metrics.bySeverity[sev]?.escalationRate ? (metrics.bySeverity[sev].escalationRate * 100).toFixed(1) + '%' : '-'}</div>
