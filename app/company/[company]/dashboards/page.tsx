      ...
      {!metrics ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            <Link href="../fast-scan">
              <KpiTile label="Total Scans" value={metrics.scans.total} clickable />
            </Link>
            <Link href="../compliance">
              <KpiTile label="Pass" value={metrics.compliance.statusCounts.pass} clickable />
            </Link>
            <Link href="../compliance">
              <KpiTile label="Warn" value={metrics.compliance.statusCounts.warn} clickable />
            </Link>
            <Link href="../compliance">
              <KpiTile label="Fail" value={metrics.compliance.statusCounts.fail} clickable />
            </Link>
          </div>
          {/* Recent Violations */}
          <div>
...
function KpiTile({ label, value, clickable }: { label: string; value: number; clickable?: boolean }) {
  return (
    <div className={`bg-white rounded shadow p-4 flex flex-col items-center transition cursor-pointer ${clickable ? 'hover:bg-blue-50 active:scale-[.98]' : ''}`}>
...
function RecentFailuresTable({ failures }: { failures: DashboardOverviewMetrics['compliance']['recentFailures'] }) {
  if (!failures.length) return <div className="text-gray-500">No recent failures.</div>
  return (
      ...
      <tbody>
        {failures.map(f => (
          <tr key={f.complianceResultId}>
            <td className="px-4 py-2">{new Date(f.occurredAt).toLocaleString()}</td>
            <td className="px-4 py-2">{f.reason}</td>
            <td className="px-4 py-2 text-xs text-gray-600">{f.evidenceIds.join(', ')}</td>
          </tr>
        ))}
      </tbody>
    </table>
