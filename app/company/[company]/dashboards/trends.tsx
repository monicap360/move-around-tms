...
export default function TrendsDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/dashboards/trends?from=${from.toISOString().slice(0, 10)}&to=${now.toISOString().slice(0, 10)}&groupBy=${groupBy}`,
      ...
      <div className="flex gap-4 mb-4">
        ...
        {Object.keys(SEVERITY_COLORS).map((sev) => (
          <label key={sev} className="ml-2">
            <input
              type="checkbox"
              checked={showSeverity[sev]}
              onChange={(e) =>
                setShowSeverity((s) => ({ ...s, [sev]: e.target.checked }))
              }
            />
            <span className="ml-1" style={{ color: SEVERITY_COLORS[sev] }}>
              {sev}
            </span>
          </label>
        ))}
...
function TrendChart({
  ...
  buckets: any[];
  type: string;
  showSeverity: Record<string, boolean>;
}) {
  ...
  const max = Math.max(
    1,
    ...buckets.map((b) =>
      allSev.reduce((sum, sev) => sum + (b.bySeverity[sev]?.[type] || 0), 0),
    ),
  );
  return (
    <div>
      ...
      >
        ...
        {buckets.map((b, i) => (
          <text
            key={b.date}
            x={pad + i * ((width - 2 * pad) / (buckets.length - 1 || 1))}
            y={height - pad + 16}
            fontSize={10}
            textAnchor="middle"
          >
      ...
      <div className="flex gap-4 mt-2">
        {allSev.map((sev) => (
          <span key={sev} className="flex items-center">
            <span
              style={{
