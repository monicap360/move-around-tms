  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        testResults.push({
          ...
          details: userError || "Connection established (role detection limited)",
    ...
    try {
      ...
      testResults.push({
        ...
        result: error ? "❌ No access to storage schema" : "✅ Can access storage schema",
        details: error || `Found storage schema: ${data?.length > 0 ? 'Yes' : 'No'}`,
    ...
    try {
      ...
      testResults.push({
        ...
        result: error ? "❌ Cannot access objects table info" : "✅ Table accessible",
        details: error || data,
        status: error ? 'error' : 'success'
      });
    } catch (err) {
    ...
    try {
      ...
      testResults.push({
        ...
        result: error ? "❌ Cannot check indexes" : `✅ Found ${data?.length || 0} indexes`,
        details: error || data?.map(idx => ({ name: idx.indexname, definition: idx.indexdef })),
        status: error ? 'error' : 'success'
      });
    } catch (err) {
      testResults.push({
    ...
    try {
      ...
      testResults.push({
        ...
        result: error ? "❌ Cannot check target index" : 
                data?.length > 0 ? "✅ Index already exists!" : "⚠️ Index not created yet",
        details: error || (data?.length > 0 ? data[0] : "Index needs to be created"),
    ...
    try {
      ...
      testResults.push({
        ...
        result: error ? "❌ Cannot test performance" : `✅ Listed files in ${duration}ms`,
        details: error || { 
          duration_ms: duration,
          file_count: files?.length || 0,
          performance_note: duration > 1000 ? "Slow - index would help significantly" : 
                           duration > 500 ? "Moderate - index would help" : "Fast"
        },
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <Card className="mb-6">
        <CardHeader>
          ...
          <p className="text-sm text-gray-600">
            Use this to diagnose database role and permission issues before creating the performance index
          </p>
        </CardHeader>
            ...
            <CardContent>
              <div className="space-y-4">
                {results.map((result, index) => (
                  <div key={index} className={`border rounded p-4 ${getStatusColor(result.status)}`}>
      ...
      <Card className="mt-6 border-blue-200 bg-blue-50">
        <CardHeader>
          <CardTitle className="text-blue-800">Next Steps Based on Results</CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-blue-700">
          <div className="space-y-4">
            <div>
              <strong>✅ If all tests pass:</strong> Ready to create the index using Monica's SQL commands
            </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside mt-2 ml-4">
                ...
                <li>Use the performance test page to verify speed improvements</li>
                <li>Run: <code>analyze storage.objects;</code> to update statistics</li>
              </ul>
            </div>
          </div>
        </CardContent>
