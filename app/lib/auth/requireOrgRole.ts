// IMPORTANT: self-contained copy of lib/auth/requireOrgRole.ts (root). Do NOT
// replace this with `export * from "@/lib/auth/requireOrgRole"` — on Linux the
// "@" alias resolved that re-export back to THIS file (circular) and the build
// failed with "requireOrgRole is not exported" (passed on case-insensitive
// Windows, failed on Render/Linux). Keep this file identical to the root module.
