...
export default function BillingHistoryPage() {
  ...
  useEffect(() => {
    async function fetchPayments() {
      ...
      const { data, error } = await supabase.from("payments").select("id, file_url, status, created_at").eq("user_id", user.id).order("created_at", { ascending: false });
  ...
  return (
    <main className="max-w-2xl mx-auto py-10">
      ...
      <div className="border rounded p-4 bg-white shadow">
        {loading ? <div>Loading…</div> : error ? <div className="text-red-600">{error}</div> : payments.length === 0 ? <div>No payment records found.</div> : (
