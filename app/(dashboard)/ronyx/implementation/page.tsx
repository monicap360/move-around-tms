...
const AUDIT_LOG = [
  ...
  { ts:"Jun 13 · 15:30", user:"System",    action:"Onyx AI readiness scan completed",     module:"All",        result:"Readiness: 68%"        },
...
const IMPORT_PHASES: ImportPhase[] = [
  ...
  { key:"projects",          label:"Projects / Jobs",   subtitle:"Active projects and jobs you run",                          icon:"📁", csvHeaders:["name","code","description","status"],                      exampleRow:["Galveston Port Expansion","GPX-2026","Phase 2 aggregate hauling","active"], endpoint:"/api/ronyx/implementation/import", table:"projects" },
  { key:"materials",         label:"Materials & Rates", subtitle:"Material types and default rates",                          icon:"🪨", csvHeaders:["material_name","unit_type","default_rate"],                exampleRow:["Crushed Limestone","Ton","14.50"],                                          endpoint:"/api/ronyx/implementation/import", table:"materials" },
  { key:"job_sites",         label:"Job Sites & Locations", subtitle:"Pickup pits, plants, and dropoff sites",                icon:"📍", csvHeaders:["vendor_name","location_name","location_type","city","state"], exampleRow:["Vulcan Materials","Vulcan Galveston Pit","pit","Galveston","TX"],             endpoint:"/api/ronyx/implementation/import", table:"vendor_locations" },
...
export default function CustomerLaunchCenterPage() {
  ...
  // Read a CSV or Excel file into CSV text. Excel (.xlsx/.xls) is converted
  // server-side via /api/ronyx/excel-parse so the same import pipeline is used.
  async function fileToCsvText(file: File): Promise<string> {
    const name = file.name.toLowerCase();
    if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
      const fd = new FormData(); fd.append("file", file);
      const res = await fetch("/api/ronyx/excel-parse", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not read this Excel file");
      return data.csv || "";
    }
          ...
          <div style={{ display:"flex", gap:16, alignItems:"flex-start" }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3, flexWrap:"wrap" as const }}>
                <span style={{ fontWeight:800, fontSize:16, color:PURP }}>Onyx — Your Launch Guide™</span>
              ...
              <div style={{ fontSize:12, color:MUTE, marginBottom:12 }}>Onyx is an <b>AI assistant</b> that watches your setup progress, finds missing information, and recommends the next best action. For anything it can't handle, message a real person below.</div>
              ...
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" as const }}>
                ...
                <button style={{ background:"rgba(139,92,246,0.14)", color:PURP, border:"1px solid rgba(139,92,246,0.28)", padding:"9px 18px", borderRadius:8, fontSize:13, fontWeight:700, cursor:"pointer" }}>✨ Ask Onyx (AI)</button>
