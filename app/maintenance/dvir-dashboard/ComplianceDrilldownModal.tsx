...
export default function ComplianceDrilldownModal({ open, onClose, dvirs, complianceMap }) {
  ...
  let filtered = dvirs;
  if (vehicle) filtered = filtered.filter(d => (d.truck_number || "").toLowerCase().includes(vehicle.toLowerCase()));
  if (driver) filtered = filtered.filter(d => (d.driver_name || "").toLowerCase().includes(driver.toLowerCase()));
  if (compliance !== "all") {
    filtered = filtered.filter(d => {
      const c = complianceMap[d.id] || { missingFields: [], invalidDefects: [], intervalOk: true };
      const isCompliant = c.missingFields.length === 0 && c.invalidDefects.length === 0 && c.intervalOk;
              ...
              {filtered.map((d, i) => {
                ...
                return (
                  <tr key={d.id || i} className={compliant ? "bg-green-50" : "bg-red-50"}>
                    ...
                    <td className={"border px-2 py-1 font-bold " + (compliant ? "text-green-700" : "text-red-700")}>{compliant ? "Compliant" : "Noncompliant"}</td>
                    <td className="border px-2 py-1 text-xs">{d.remarks}</td>
                  </tr>
                );
              })}
              {filtered.length === 0 && <tr><td colSpan={6} className="text-center py-4 text-gray-400">No DVIRs found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
