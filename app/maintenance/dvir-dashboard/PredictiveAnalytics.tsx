...
function calculateRiskScore(dvirs, truck) {
  ...
  if (
    !lastInspection ||
    now.getTime() -
      new Date(lastInspection.date || lastInspection.created_at).getTime() >
      1000 * 60 * 60 * 24 * 30
  ) {
    score += 20;
...
export default function PredictiveAnalytics({ dvirs }) {
  ...
  const trucks: string[] = useMemo(
    () => Array.from(new Set(dvirs.map((d: any) => String(d.truck_number)))),
    [dvirs],
  );
          ...
          <button
            ...
            onClick={() => {
              if (sectionRef.current)
                exportNodeAsPng(
                  sectionRef.current,
                  `predictive_analytics_${new Date().toISOString().slice(0, 10)}.png`,
                );
            }}
          >
            Export as Image
          ...
          <CardContent className="py-4">
            ...
            <ul className="list-disc ml-6 text-sm mb-4">
              ...
              {highRisk.map((r: RiskData) => (
                <li key={r.truck}>
                  Truck <b>{r.truck}</b> (Risk Score: {r.score}) -{" "}
                  {r.reasons.join(", ")}
                </li>
              ))}
            </ul>
