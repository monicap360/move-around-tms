...
async function generateDriverApplicationForm(): Promise<Uint8Array> {
  ...
  page.drawText(
    "Please complete all sections. Print clearly in blue or black ink.",
    {
      x: 50,
      y: yPos,
      size: 10,
  ...
  for (const field of employmentFields) {
    if (field === "") {
      ...
      page.drawText(field, {
        ...
        size: 9,
        font: font,
        color: field.includes(":") ? black : gray,
      });
      yPos -= 12;
    }
  }
  ...
  for (const field of referenceFields) {
    if (field === "") {
      ...
      page2.drawText(field, {
        ...
        size: 9,
        font: font,
        color: field.includes(":") && !field.includes("Name:") ? black : gray,
      });
      yPos2 -= 15;
    }
  }
...
async function generateDirectDepositForm(): Promise<Uint8Array> {
  ...
  page.drawText(
    "Please attach a voided check or deposit slip for verification.",
    {
      x: 50,
      y: yPos,
      size: 9,
...
async function generateEquipmentChecklistForm(): Promise<Uint8Array> {
  ...
  for (const item of equipmentItems) {
    if (Array.isArray(item)) {
      ...
      const textFont =
        item.includes("CHECKLIST") ||
        item.includes("CONDITION") ||
        item.includes("RESPONSIBILITIES")
          ? fontBold
          : font;
      page.drawText(item, {
        x: 50,
  ...
  page.drawText(
    "Driver Signature: _____________________________  Date: ______________",
    {
      x: 50,
      y: yPos,
      size: 10,
