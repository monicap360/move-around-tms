...
export async function POST(req: Request) {
  try {
    ...
    const vsData: any = { organization_id: orgId ?? "00000000-0000-0000-0000-000000000000", provider: "saferwatch", owner_operator_id: ooId, mc_number: mcNumber, dot_number: dotNumber, raw_response: raw, normalized_data: normalized, verification_status: normalized.overall_status === "clear" ? "clear" : normalized.overall_status === "blocked" ? "blocked" : "needs_attention", authority_status: normalized.authority.status, safety_status: normalized.safety.rating };
