...
export async function POST(req: NextRequest) {
  ...
  const { form_type, period_start, period_end, filing_date, status, uploaded_pdf, submitted_by } = body
  const { data, error } = await supabaseAdmin
    .from('payroll_tax_filings')
    .insert([{ form_type, period_start, period_end, filing_date, status, uploaded_pdf, submitted_by }])
    .select('*, employees!payroll_tax_filings_submitted_by_fkey(full_name)')
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
