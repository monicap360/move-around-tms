...
export async function GET(req: NextRequest) {
  ...
  if (tripIds.length > 0) {
    const { data: segs } = await supabaseAdmin
      ...
      .in("trip_id", tripIds);
    for (const s of segs || []) {
      const c = s.jurisdiction_code || "UNK";
      milesByJuris[c] = (milesByJuris[c] || 0) + Number(s.miles || 0);
    }
  }
  const { data: fuel } = await supabaseAdmin
    .from("fuel_purchases")
  ...
  let totalGallons = 0;
  for (const f of fuel || []) {
    const c = f.jurisdiction_code || "UNK";
    const g = Number(f.gallons || 0);
    gallonsByJuris[c] = (gallonsByJuris[c] || 0) + g;
    totalGallons += g;
  }
  ...
  const rateMap = new Map(
    (rates || []).map((r) => [
      r.jurisdiction_code,
      Number(r.rate_per_gallon || 0),
    ]),
  );

  const codes = Array.from(
    new Set([...Object.keys(milesByJuris), ...Object.keys(gallonsByJuris)]),
  ).filter((c) => (jurisdictionFilter ? c === jurisdictionFilter : true));
  ...
  const draw = (y: number, text: string, size = 12) => {
    page.drawText(text, { x: 40, y, size, font });
  };

  let y = 740;
  draw(y, `IFTA Quarterly Summary (TX Filing) — Q${quarter} ${year}`, 16);
  y -= 24;
  draw(y, `Period: ${from} to ${to}`);
  y -= 16;
  draw(
    y,
    `Fleet MPG: ${fleetMPG.toFixed(3)}  |  Total Miles: ${totalMiles.toFixed(1)}  |  Total Gallons: ${totalGallons.toFixed(3)}`,
  );
  y -= 22;
  draw(
    y,
    "Jurisdiction  Miles     Gal Purch  Taxable Gal  Rate     Tax Due   Tax Paid   Net Tax",
  );
  y -= 14;
  draw(
    y,
  ...
  for (const r of rows) {
    ...
    draw(y, line, 10);
    y -= 12;
    totals.miles += r.miles;
    totals.gp += r.gallonsPurchased;
    totals.tg += r.taxableGallons;
    totals.due += r.taxDue;
    totals.paid += r.taxPaid;
    totals.net += r.netTax;
    if (y < 80) {
      y = 740;
      pdf.addPage([612, 792]);
  ...
  draw(
    y,
    `TOTALS        ${fmt(totals.miles, 1).padStart(9)}  ${fmt(totals.gp, 3).padStart(10)}  ${fmt(totals.tg, 3).padStart(11)}           ${fmt(totals.due).padStart(8)}  ${fmt(totals.paid).padStart(9)}  ${fmt(totals.net).padStart(8)}`,
    10,
  );
  ...
  return new NextResponse(Buffer.from(bytes), {
    ...
    headers: {
      ...
      "Content-Disposition": `inline; filename="ifta-q${quarter}-${year}.pdf"`,
    },
  });
}
