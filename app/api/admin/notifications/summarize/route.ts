...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, message: "Failed to summarize" },
      { status: 500 },
    );
  }
}
...
function summarizeHeuristically(
  list: { message: string; created_at: string }[],
) {
  ...
  for (const n of list) {
    const m1 =
      n.message.match(/Document\s([^\s!]+)/i) ||
      n.message.match(/Expiring document:\s([^\s]+)/i);
    const docType = m1?.[1]?.replace(/[:]/g, "") || "Other";
    docTypeCounts[docType] = (docTypeCounts[docType] || 0) + 1;
    const m2 =
      n.message.match(/on\s(\d{4}-\d{2}-\d{2})/) ||
      n.message.match(/(\d{4}-\d{2}-\d{2})/);
    const d = m2?.[1];
    if (d) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful assistant for HR compliance summaries.",
        },
