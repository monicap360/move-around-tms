...
export async function GET(req: NextRequest) {
  ...
  let q = supabaseAdmin
    ...
    .select(
      "id, ticket_date, partner_id, driver_id, material, unit_type, quantity, pay_rate, bill_rate, total_pay, total_bill, total_profit, voided, status, aggregate_partners(name)",
    )
  ...
  const items = (data || []).map((t: any) => ({
    ...
    margin_pct:
      Number(t.total_bill || 0) > 0
        ? (Number(t.total_profit || 0) / Number(t.total_bill || 0)) * 100
        : 0,
    voided: !!t.voided,
    status: t.status,
  }));
  ...
  if (groupBy !== "none") {
    ...
    groups = Array.from(m.values()).map((g) => ({
      ...g,
      margin_pct: g.bill > 0 ? (g.profit / g.bill) * 100 : 0,
    }));
  ...
  return NextResponse.json({
    ok: true,
    items,
    summary: { totals: { ...totals, margin_pct }, groups },
  });
}
