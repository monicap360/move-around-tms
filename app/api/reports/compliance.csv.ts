...
import { fetchComplianceResults, fetchScans, fetchDocuments } from '@/src/dashboard/data.adapters'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = [
      ['Window', 'Pass', 'Warn', 'Fail', 'Top Violations', 'Expiring Docs', 'Total Scans', 'Avg OCR Confidence'],
      [
        ...
        metrics.compliance.topViolations.map(v => `${v.code} (${v.count})`).join('; '),
    ...
    return NextResponse.json({ error: err.message ?? 'Failed to generate report' }, { status: 400 })
  }
}
