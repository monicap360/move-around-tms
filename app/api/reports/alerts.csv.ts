...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = [
      ['Alert ID', 'Severity', 'Triggered At', 'Acknowledged At', 'Time to Ack (s)', 'Evidence Refs', 'Message', 'Metric Path'],
      ...events.map(e => [
        e.alertId,
        e.severity,
        e.triggeredAt,
        e.acknowledgedAt || '',
        e.acknowledgedAt ? Math.round((new Date(e.acknowledgedAt).getTime() - new Date(e.triggeredAt).getTime()) / 1000) : '',
        e.evidenceRefs.join(';'),
        e.message,
        e.metricPath,
      ])
