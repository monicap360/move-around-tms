import { NextRequest, NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";

export const dynamic = "force-dynamic";

function createServerAdmin() {
  return createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!,
    { auth: { persistSession: false } }
  );
}

async function refreshQuickBooksToken(refreshToken: string) {
  const clientId = process.env.QUICKBOOKS_CLIENT_ID;
  const clientSecret = process.env.QUICKBOOKS_CLIENT_SECRET;

  if (!clientId || !clientSecret) {
    throw new Error("QuickBooks OAuth is not configured.");
  }

  const tokenResponse = await fetch("https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer", {
    method: "POST",
    headers: {
      Authorization: `Basic ${Buffer.from(`${clientId}:${clientSecret}`).toString("base64")}`,
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json",
    },
    body: new URLSearchParams({
      grant_type: "refresh_token",
      refresh_token: refreshToken,
    }).toString(),
  });

  const payload = await tokenResponse.json();
  if (!tokenResponse.ok) {
    throw new Error(payload?.error_description || "QuickBooks token refresh failed.");
  }

  return payload;
}

export async function POST(req: NextRequest) {
  try {
    const { customerId, amount, lineDescription, docNumber, organizationId } = await req.json();

    if (!customerId || !amount) {
      return NextResponse.json({ error: "Missing customerId or amount." }, { status: 400 });
    }

    let accessToken = process.env.QUICKBOOKS_ACCESS_TOKEN;
    let realmId = process.env.QUICKBOOKS_REALM_ID;
    let itemId = process.env.QUICKBOOKS_ITEM_ID;

    if (organizationId) {
      const supabase = createServerAdmin();
      const { data: integration } = await supabase
        .from("accounting_integrations")
        .select("access_token, refresh_token, realm_id, expires_at, active")
        .eq("organization_id", organizationId)
        .eq("provider", "quickbooks")
        .eq("active", true)
        .single();

      if (integration?.access_token) {
        accessToken = integration.access_token;
      }
      if (integration?.realm_id) {
        realmId = integration.realm_id;
      }

      if (integration?.refresh_token && integration?.expires_at) {
        const expiresAt = new Date(integration.expires_at).getTime();
        const shouldRefresh = Number.isFinite(expiresAt) && expiresAt - Date.now() < 5 * 60 * 1000;
        if (shouldRefresh) {
          const refreshed = await refreshQuickBooksToken(integration.refresh_token);
          accessToken = refreshed.access_token || accessToken;
          const newExpiresAt = refreshed.expires_in
            ? new Date(Date.now() + refreshed.expires_in * 1000).toISOString()
            : integration.expires_at;

          await supabase
            .from("accounting_integrations")
            .update({
              access_token: refreshed.access_token,
              refresh_token: refreshed.refresh_token || integration.refresh_token,
              expires_at: newExpiresAt,
              updated_at: new Date().toISOString(),
            })
            .eq("organization_id", organizationId)
            .eq("provider", "quickbooks");
        }
      }
    }

    if (!accessToken || !realmId || !itemId) {
      return NextResponse.json(
        {
          error:
            "QuickBooks is not configured. Connect QuickBooks or set QUICKBOOKS_ACCESS_TOKEN, QUICKBOOKS_REALM_ID, QUICKBOOKS_ITEM_ID.",
        },
        { status: 400 }
      );
    }

    const payload = {
      Line: [
        {
          Amount: Number(amount),
          DetailType: "SalesItemLineDetail",
          Description: lineDescription || "Load service",
          SalesItemLineDetail: {
            ItemRef: { value: itemId },
            Qty: 1,
            UnitPrice: Number(amount),
          },
        },
      ],
      CustomerRef: { value: String(customerId) },
      DocNumber: docNumber,
      TxnDate: new Date().toISOString().split("T")[0],
    };

    const response = await fetch(`https://quickbooks.api.intuit.com/v3/company/${realmId}/invoice?minorversion=65`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: JSON.stringify(payload),
    });

    const data = await response.json();
    if (!response.ok) {
      return NextResponse.json({ error: data?.Fault || data }, { status: response.status });
    }

    return NextResponse.json({ invoice: data });
  } catch (error: any) {
    return NextResponse.json({ error: error.message || "QuickBooks request failed." }, { status: 500 });
  }
}
