...
export async function POST(request: NextRequest) {
  try {
    ...
    const defectiveItems = inspection_items.filter((item: any) => 
...
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, defects_corrected, mechanic_signature, correction_notes } = body;
    ...
    if (defects_corrected) {
      const { error: defectsUpdateError } = await supabaseAdmin
        ...
        .update({
          ...
          correction_notes: correction_notes || 'Marked as corrected via DVIR dashboard'
        })
        .eq('dvir_id', id);
...
function determineSeverity(category: string, item: string): string {
  const criticalItems = [
    'brake', 'steering', 'tire', 'wheel', 'suspension'
  ];
  
  const highItems = [
    'light', 'signal', 'mirror', 'horn', 'windshield'
  ];
