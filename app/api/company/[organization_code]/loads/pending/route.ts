...
export async function GET(request: Request) {
  try {
    ...
    if (!orgCode) {
      return NextResponse.json({ ok: false, error: 'Missing organization code' }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: false, error: err.message || 'Failed to fetch pending loads' }, { status: 500 });
  }
}
