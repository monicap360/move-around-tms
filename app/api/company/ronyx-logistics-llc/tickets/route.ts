...
export async function POST(request: Request) {
  try {
    ...
    const ticketNumber = body.ticket_number || `TKT-${Date.now().toString().slice(-8)}`;
    ...
    return NextResponse.json({ ok: false, error: err.message || 'Failed to create ticket' }, { status: 500 });
  }
}
