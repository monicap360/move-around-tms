...
export async function POST(request: Request) {
  try {
    ...
    let truckQuery = supabaseAdmin
      ...
      .select('*')
      .eq('status', 'Ready');
    if (required_truck_type) truckQuery = truckQuery.eq('truck_type', required_truck_type);
    const { data: trucks, error: truckError } = await truckQuery.limit(1);
    if (truckError) throw truckError;
    if (!trucks || trucks.length === 0) {
      return NextResponse.json({ ok: false, message: 'No available truck found.' }, { status: 200 });
    }
    ...
    return NextResponse.json({ ok: false, message: err.message || 'Dispatch auto-assign failed' }, { status: 500 });
  }
}
