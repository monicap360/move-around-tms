...
export async function GET() {
  const { data } = await supabaseAdmin.from("ccb_users").select("id, name, role").eq("active", true).order("name");
...
export async function POST(req: Request) {
  ...
  const { data: u } = await supabaseAdmin.from("ccb_users").select("*").eq("id", id).eq("active", true).maybeSingle();
  if (u && u.pin_hash && crypto.timingSafeEqual(Buffer.from(u.pin_hash), Buffer.from(hashPin(pin, u.salt)))) {
    ...
    res.cookies.set("ccb_session", token, { httpOnly: true, secure: true, sameSite: "lax", path: "/", maxAge: SESSION_HOURS * 3600 });
    return res;
  }
