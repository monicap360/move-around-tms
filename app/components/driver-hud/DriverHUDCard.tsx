...
export const DriverHUDCard: React.FC<DriverHUDCardProps> = ({
  ...
  <div className="bg-black/80 rounded-2xl shadow-2xl p-6 flex flex-col items-center gap-4 border border-cyan-800 relative group hover:scale-105 transition-transform cursor-pointer">
    <div className="flex gap-6 items-center">
      <DriverPhoto photoUrl={photoUrl} name={name} editable={editable} onChangePhoto={onChangePhoto} />
      <TruckLogoBadge logoUrl={logoUrl} editable={editable} onChangeLogo={onChangeLogo} />
    </div>
    <div className="mt-2 text-cyan-200 text-xl font-bold text-center">{name}</div>
    <div className="flex flex-wrap gap-3 justify-center mt-2">
      {truckNumber && (
        <span className="bg-cyan-900 text-cyan-200 px-3 py-1 rounded-full text-xs font-semibold">Truck #{truckNumber}</span>
      )}
      {yard && (
        <span className="bg-cyan-800 text-cyan-100 px-3 py-1 rounded-full text-xs font-semibold">Yard: {yard}</span>
      )}
      {lastTicket && (
        <span className="bg-cyan-700 text-cyan-100 px-3 py-1 rounded-full text-xs font-semibold">Last Ticket: {lastTicket}</span>
      )}
      {typeof aiRiskScore === "number" && (
        <span className="bg-cyan-600 text-cyan-50 px-3 py-1 rounded-full text-xs font-semibold">AI Risk: {aiRiskScore}</span>
      )}
    </div>
    {editable && (
      <button
