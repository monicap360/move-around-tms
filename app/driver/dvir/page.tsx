...
export default function DVIRPage() {
  ...
  const [currentCategory, setCurrentCategory] =
    useState<string>("Engine Compartment");
  ...
  const getCurrentCategoryItems = () => {
    return currentDVIR.inspection_items.filter(
      (item) => item.category === currentCategory,
    );
  };

  const getCategoryStatus = (category: string) => {
    const categoryItems = currentDVIR.inspection_items.filter(
  ...
  const getOverallInspectionStatus = () => {
    ...
    const defectiveItems = allItems.filter(
      (item) => item.status === "defective",
    );
    const uninspectedItems = allItems.filter(
  ...
  const submitDVIR = async () => {
    ...
    try {
      ...
      const defectiveItems = currentDVIR.inspection_items.filter(
        (item) => item.status === "defective",
      );
      ...
      if (defectiveItems.length > 0) {
        ...
        if (maintenanceError) {
          console.error(
            "Error creating maintenance requests:",
            maintenanceError,
          );
        }
      }
      ...
      alert(
        `DVIR submitted successfully! ${defectiveItems.length > 0 ? `${defectiveItems.length} maintenance request(s) created.` : ""}`,
      );
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              DVIR Summary -{" "}
              {currentDVIR.inspection_type.replace("_", "-").toUpperCase()}
            </CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {currentDVIR.odometer_reading.toLocaleString()} mi
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-600">
            ...
            {defectiveItems.length > 0 ? (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <p className="text-sm text-red-700 mt-3 font-medium">
                  ⚠️ This vehicle should NOT be operated until defects are
                  corrected!
                </p>
              </div>
            ) : (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCurrentDVIR((prev) => ({
                      ...prev,
                      inspection_type: e.target.value as
                        | "pre_trip"
                        | "post_trip",
                    }))
                  }
                ...
                <div key={item.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          updateInspectionItem(item.id, { status: "defective" })
                        }
                        className={`px-3 py-1 rounded text-sm font-medium flex items-center gap-1 ${
