  ...
  return (
    <html lang="en">
      <head>
        ...
        <meta name="description" content={`Enterprise Transportation Management System for ${branding.companyName}`} />
        <link rel="manifest" href="/manifest.json" />
        <link rel="icon" href={branding.faviconUrl} />
        <link rel="apple-touch-icon" href="/icons/icon-192x192.png" />
        <meta name="apple-mobile-web-app-capable" content="yes" />
        <meta name="apple-mobile-web-app-status-bar-style" content="default" />
        <meta name="apple-mobile-web-app-title" content={branding.companyName} />
      ...
      <body className="h-screen bg-gray-100 text-gray-800">
        ...
        <header className="w-full flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200">
          <img src={branding.logoUrl} alt="Logo" className="h-8 w-8 rounded mr-2" style={{background:'white'}} />
          <span className="font-bold text-lg" style={{color:branding.primaryColor}}>{branding.companyName}</span>
        </header>
        <SupabaseProvider>
          <LoadingProvider>
            <ClientLayoutWrapper>
