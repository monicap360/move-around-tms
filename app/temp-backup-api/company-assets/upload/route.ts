...
export async function POST(request: NextRequest) {
  try {
    ...
    const assetType = formData.get("asset_type") as
      | "company_logo"
      | "ticket_template";
    ...
    if (
      !assetType ||
      !["company_logo", "ticket_template"].includes(assetType)
    ) {
      return NextResponse.json(
    ...
    const publicUrl = await CompanyAssetsStorageService.getPublicUrl(
