...
export default function ContractModal({ open, onClose, contract, onSign }: {
  ...
  useEffect(() => {
    async function fetchAudit() {
      ...
      let { data: loadAudit } = await supabase.from('contract_audit').select('*').eq('load_id', contract.id).order('timestamp', { ascending: false });
      let { data: partnerAudit } = await supabase.from('compliance_violations').select('*').eq('partner_id', contract.id).order('timestamp', { ascending: false });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-lg w-full relative">
        ...
        <div className="mb-4">
          <span className={contract.signed ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}>
            Status: {contract.signed ? 'Signed' : 'Not Signed'}
          </span>
        </div>
        <div className="flex gap-2 mb-4">
          <button className="bg-gray-200 px-4 py-1 rounded" onClick={handleDownloadPDF}>Download PDF</button>
          {!contract.signed && (
            ...
            <ul className="text-xs max-h-32 overflow-y-auto">
              {audit.map((a, i) => (
                <li key={i} className="mb-1">
                  <span className="font-mono text-gray-500">{a.timestamp || a.signed_at || a.created_at}:</span> {a.action || a.reason}
                </li>
              ))}
            </ul>
          )}
