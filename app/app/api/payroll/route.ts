...
export async function GET(request: Request) {
  try {
    ...
    if (!expected) {
      console.warn('ADMIN_TOKEN is not set in environment; /api/payroll is unprotected')
    }
    ...
    const totals = data.reduce(
      (acc: any, row: any) => ({
        ...
        social_security_withholding: acc.social_security_withholding + (Number(row.social_security_withholding) || 0),
        medicare_withholding: acc.medicare_withholding + (Number(row.medicare_withholding) || 0),
        federal_tax_withholding: acc.federal_tax_withholding + (Number(row.federal_tax_withholding) || 0),
        employer_social_security: acc.employer_social_security + (Number(row.employer_social_security) || 0),
        employer_medicare: acc.employer_medicare + (Number(row.employer_medicare) || 0),
    ...
    return NextResponse.json({ ok: true, weekStart, rows: data, summary: totals })
  } catch (err) {
