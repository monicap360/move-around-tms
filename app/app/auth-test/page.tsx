  ...
  const testEmailLogin = async () => {
    ...
    try {
      ...
      if (data?.session) {
        ...
        setResult((prev: any) => ({
          ...prev,
          sessionCheck: sessionCheck.data?.session ? 'Session confirmed' : 'No session found'
        }))
      }
    } catch (err) {
  ...
  const testGetSession = async () => {
    ...
    try {
      ...
      setResult({ 
        type: 'session', 
        success: !error, 
        data: data?.session ? `Session: ${data.session.user?.email}` : 'No session',
        error: error?.message 
      })
    } catch (err) {
    ...
    <div className="max-w-2xl mx-auto p-8">
      ...
      {result && (
        <div className="mb-4 p-4 border rounded">
          <h3 className="font-bold mb-2">
            Test Result ({result.type}): {result.success ? '✅ Success' : '❌ Failed'}
          </h3>
          {result.data && <p><strong>Data:</strong> {result.data}</p>}
          {result.sessionCheck && <p><strong>Session Check:</strong> {result.sessionCheck}</p>}
      ...
      <div className="mt-8 p-4 bg-gray-100 rounded">
        ...
        <ol className="list-decimal list-inside space-y-1 text-sm">
          ...
          <li>If not logged in, try "Test Email Login" (use actual credentials)</li>
