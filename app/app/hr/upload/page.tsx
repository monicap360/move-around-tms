...
export default function HRUpload() {
    ...
    try {
        ...
        try {
          const assetResponse = await fetch('/api/company-assets', {
            ...
            body: JSON.stringify({
              ...
              description: description || `${uploadType.replace('_', ' ')} uploaded ${new Date().toLocaleDateString()}`,
              file_size: file.size,
              mime_type: file.type,
              tags: uploadType === 'company_logo' ? ['branding', 'official'] : 
                   uploadType === 'ticket_template' ? ['template', 'billing'] : 
                   ['template', 'form'],
              metadata: {
                width: uploadType === 'company_logo' && file.type.startsWith('image/') ? 'auto-detected' : null,
                height: uploadType === 'company_logo' && file.type.startsWith('image/') ? 'auto-detected' : null,
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/ocr-scan`,
        {
          ...
          body: JSON.stringify({ kind: 'hr', file_url: signed.signedUrl, driverId: driverId || undefined }),
        }
      );
  ...
  const uploadTypeConfig = {
    driver_document: {
      ...
      description: "Upload driver licenses, medical certificates, or other HR documents",
      icon: FileText,
    ...
    company_logo: {
      title: "Company Logo",
      description: "Upload Ronyx Logistics LLC company logo for use in documents and forms",
      icon: Building,
      accept: "image/*",
      showDriverId: false
    },
    ticket_template: {
      ...
      description: "Upload blank ticket templates for reference and form generation",
      icon: Truck,
      accept: "image/*,application/pdf",
      showDriverId: false
    },
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 to-white p-8 text-gray-800">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-600 mb-6">3741 Graves Ave, Groves, Texas 77619</p>
        
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Document & Asset Upload Center</h2>
        ...
        <Card className="mt-8 max-w-2xl mx-auto">
          ...
          <CardContent className="text-sm text-gray-600 space-y-2">
            <p><strong>Company Logo:</strong> Upload high-resolution logo files (PNG, JPG, SVG) for use in documents, forms, and system branding.</p>
            <p><strong>Ticket Templates:</strong> Upload blank ticket templates to help analyze required fields and generate digital forms.</p>
            <p><strong>Driver Documents:</strong> Upload driver licenses, medical certificates, and other HR documents for OCR processing and compliance tracking.</p>
            <p><strong>Form Templates:</strong> Upload blank contracts, applications, or other forms for digitization and automation.</p>
          </CardContent>
        </Card>
      </div>
    </main>
