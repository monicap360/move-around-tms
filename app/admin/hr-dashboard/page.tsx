...
export default async function HRDashboardPage() {
  const { data, error } = await supabaseAdmin
    ...
    .select('id, doc_type, image_url, status, expiration_date, ocr_confidence, drivers ( id, name )')
            ...
            {docs.map((d: any) => {
              ...
              return (
                <tr key={d.id} className="odd:bg-white even:bg-gray-50">
                  <td className="p-2 border-b">{d.drivers?.name ?? 'Unassigned'}</td>
                  <td className="p-2 border-b">{d.doc_type}</td>
                  <td className="p-2 border-b">{d.expiration_date ?? '-'}</td>
                  <td className="p-2 border-b">{typeof d.ocr_confidence === 'number' ? `${d.ocr_confidence}%` : '-'}</td>
                  <td className="p-2 border-b">
                    <span className={`px-2 py-1 rounded text-xs ${d.status === 'Approved' ? 'bg-green-100 text-green-700' : d.status === 'Denied' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-800'}`}>
                      {d.status}
                    </span>
                  </td>
                  <td className="p-2 border-b">
                    {url ? (
                      <div className="flex items-center gap-2">
                        {isImage && <img src={url} alt="preview" className="h-12 w-auto object-contain border rounded" />}
