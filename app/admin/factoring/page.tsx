...
export default function FactoringAdminPage() {
  ...
  const save = async () => {
    try {
      if (form.id) {
        const data = await api<{ company: Company }>("/api/factoring", {
          method: "PATCH",
          body: JSON.stringify(form),
        });
        setRows((arr) =>
          arr.map((v) => (v.id === data.company.id ? data.company : v)),
        );
      } else {
        const data = await api<{ company: Company }>("/api/factoring", {
          method: "POST",
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <input
              ...
              type="number"
              step="0.01"
              placeholder="Advance Rate (e.g., 0.9)"
              value={form.advance_rate ?? ""}
              onChange={(e) =>
                setForm({ ...form, advance_rate: Number(e.target.value) })
              }
            />
            <input
              className="border p-2 rounded"
              type="number"
              step="0.01"
              placeholder="Fee Rate (e.g., 0.02)"
              value={form.fee_rate ?? ""}
              onChange={(e) =>
                setForm({ ...form, fee_rate: Number(e.target.value) })
              }
            />
            <input
              className="border p-2 rounded"
              type="number"
              step="0.01"
              placeholder="Reserve (e.g., 0.1)"
              value={form.reserve_rate ?? ""}
              onChange={(e) =>
                setForm({ ...form, reserve_rate: Number(e.target.value) })
              }
            />
            <input
              className="border p-2 rounded"
              ...
              onChange={(e) =>
                setForm({ ...form, standard_days: Number(e.target.value) })
              }
            />
            <input
              className="border p-2 rounded"
          ...
          <div className="flex gap-2">
            ...
            {form.id && (
              <Button variant="secondary" onClick={() => setForm({ name: "" })}>
                Cancel
              </Button>
            )}
          </div>
