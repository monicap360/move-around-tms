...
export default function IFTAAdminPage() {
  ...
  async function run() {
    ...
    try {
      const res = await fetch(
        `/api/admin/ifta/summary?year=${year}&quarter=${quarter}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      const json = await res.json();
      if (!res.ok || !json.ok)
        throw new Error(json.error || "Failed to compute IFTA");
      setRows(json.rows || []);
  ...
  function exportCsv() {
    ...
    const rowsCsv = rows.map((r) => [
      r.jurisdiction_code,
      r.miles.toFixed(1),
      r.gallons_purchased.toFixed(3),
      r.taxable_gallons.toFixed(3),
      r.rate.toFixed(4),
      r.tax_due.toFixed(2),
    ...
    const csv = [
      ...
      [
        ...
        totals.gallons_purchased.toFixed(3),
        totals.taxable_gallons.toFixed(3),
        "",
        totals.tax_due.toFixed(2),
        totals.tax_paid.toFixed(2),
        totals.net_tax.toFixed(2),
      ],
    ]
      .map((r) => r.join(","))
      .join("\n");
    ...
    a.href = url;
    a.download = `ifta-q${quarter}-${year}.csv`;
  ...
  async function exportPdf() {
    const res = await fetch(
      `/api/admin/ifta/export?year=${year}&quarter=${quarter}`,
      { headers: { Authorization: `Bearer ${token}` } },
    );
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `ifta-q${quarter}-${year}.pdf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  async function saveFiling() {
    ...
    const res = await fetch("/api/admin/ifta/filings", {
      method: "POST",
      headers: {
    ...
    const json = await res.json();
    if (!res.ok || !json.ok) {
      setError(json.error || "Failed to save filing");
    } else {
      alert("Saved filing draft.");
    }
  }
  ...
  async function loadRates() {
    if (!token) return;
    try {
      const res = await fetch("/api/admin/ifta/rates", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
  ...
  async function upsertRate() {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.ok)
        throw new Error(json.error || "Failed to save rate");
      await loadRates();
      alert("Saved rate");
    } catch (e: any) {
    ...
    <main className="p-6 space-y-6">
      ...
      <section className="bg-white p-4 rounded border shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="md:col-span-3 text-sm text-gray-600 flex items-end">
            Fleet MPG:{" "}
            <span className="font-semibold ml-1">{fleet.mpg.toFixed(3)}</span> ·
            Miles: {fleet.total_miles.toFixed(1)} · Gallons:{" "}
            {fleet.total_gallons.toFixed(3)}
          </div>
        </div>
        {error && (
      ...
      <section className="grid md:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            <CardTitle>Upload Gas Receipts (CSV)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <Button
              onClick={() =>
                uploadCsv(
                  "/api/admin/ifta/upload-fuel",
                  fuelFileRef.current?.files?.[0] || null,
                )
              }
            ...
            <p className="text-xs text-gray-600">
              CSV headers:
              purchase_date,jurisdiction_code,truck_id,vendor,gallons,amount
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Upload Mileage Logs (CSV)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <input
              ref={tripsFileRef}
            ...
            <Button
              onClick={() =>
                uploadCsv(
                  "/api/admin/ifta/upload-trips",
                  tripsFileRef.current?.files?.[0] || null,
                )
              }
              disabled={loading || !token}
            >
            ...
            <p className="text-xs text-gray-600">
              CSV headers:
              trip_id,start_time,end_time,truck_id,jurisdiction_code,miles
            </p>
          </CardContent>
        </Card>
      </section>
                ...
                {rates.length === 0 && (
                  <tr>
                    <td colSpan={3} className="p-3 text-gray-500">
                      No rates loaded. Enter Admin Token to load.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="grid md:grid-cols-4 gap-3 items-end">
            <label className="text-sm">
              ...
              <input
                ...
                value={newRate.code}
                onChange={(e) =>
                  setNewRate({ ...newRate, code: e.target.value.toUpperCase() })
                }
              />
            </label>
            <label className="text-sm">
            ...
            <label className="text-sm">
              ...
              <input
                ...
                value={newRate.rate}
                onChange={(e) =>
                  setNewRate({ ...newRate, rate: Number(e.target.value) })
                }
              />
            </label>
            <Button onClick={upsertRate} disabled={!token}>
              Save/Update
            </Button>
          </div>
          <p className="text-xs text-gray-600">
            Tip: Add Louisiana (LA) here with the latest published IFTA rate.
          </p>
        </CardContent>
      </Card>
    </main>
