...
export default function ContractsAdminPage() {
  ...
  async function create() {
    ...
    setMsg('Contract created'); setForm({ contract_type: 'W2' }); load();
  }
        ...
        <CardContent className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select className="w-full px-3 py-2 border rounded" value={form.contract_type as any} onChange={(e)=>setForm({...form, contract_type: e.target.value as any})}>
                <option>W2</option>
                <option>1099</option>
                <option>LEASE</option>
              </select>
            ...
            <div className="md:col-span-3">
              ...
              <input className="w-full px-3 py-2 border rounded" value={form.contract_url||''} onChange={(e)=>setForm({...form, contract_url: e.target.value})} />
            </div>
              ...
              <tbody>
                {items.map((it)=> (
                  <tr key={it.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      {it.contract_url ? <a className="text-blue-600 underline" href={it.contract_url} target="_blank">Open</a> : ''}
                    </td>
                  </tr>
                ))}
              </tbody>
