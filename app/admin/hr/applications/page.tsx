...
type Application = { id: string; name: string; email: string; phone?: string; position?: string; submitted_at?: string; status?: string };
...
export default function HRApplicationsPage() {
  ...
  useEffect(()=>{ (async ()=>{
    try {
      ...
      const { data, error } = await supabase.from('driver_applications').select('id,name,email,phone,position,submitted_at,status').order('submitted_at', { ascending: false });
              ...
              <tbody>
                {filtered.map((a) => (
                  <tr key={a.id} className="border-b">
                    ...
                    <td className="p-2">
                      <a className="text-blue-600 underline" href={mailtoDraft(a)}>Draft Interview Email</a>
                    </td>
                  </tr>
                ))}
                {filtered.length===0 && <tr><td colSpan={7} className="p-3 text-gray-500">No applications found (or table missing). Ensure 'driver_applications' table exists.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
