...
export default function TaxFilingsAdminPage() {
  ...
  async function load() {
    setErr("");
    const res = await fetch("/api/admin/tax-filings", {
      headers: { Authorization: `Bearer ${token}` },
    });
    const json = await res.json();
    if (!res.ok || !json.ok) {
  ...
  async function create() {
    ...
    setMsg("Filing recorded");
    setForm({ form_type: "941", status: "pending" });
    load();
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <div>
              ...
              <select
                ...
                value={form.form_type as any}
                onChange={(e) =>
                  setForm({ ...form, form_type: e.target.value as any })
                }
              >
                <option>940</option>
                <option>941</option>
            ...
            <div>
              ...
              <select
                className="w-full px-3 py-2 border rounded"
                value={(form.status as any) || "pending"}
                onChange={(e) =>
            ...
            <div className="md:col-span-3">
              ...
              <input
                className="w-full px-3 py-2 border rounded"
                value={form.uploaded_pdf || ""}
                onChange={(e) =>
                  setForm({ ...form, uploaded_pdf: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <select
                className="w-full px-3 py-2 border rounded"
                value={(form.submitted_by as any) || ""}
                onChange={(e) =>
              ...
              <tbody>
                {items.map((it) => (
                  <tr key={it.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      {(it as any).employees?.full_name || ""}
                    </td>
                    <td className="p-2">
                      {it.uploaded_pdf ? (
                        <a
                          className="text-blue-600 underline"
                          href={it.uploaded_pdf}
                          target="_blank"
                        >
                          Open
                        </a>
