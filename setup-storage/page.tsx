    ...
    try {
      ...
      for (const bucketConfig of requiredBuckets) {
        ...
        if (bucketExists) {
          ...
          if (accessError) {
            addResult(
              `⚠️  ${bucketConfig.name} exists but access error: ${accessError.message}`,
            );
          } else {
          ...
          if (createError) {
            addResult(
              `❌ Failed to create ${bucketConfig.name} bucket: ${createError.message}`,
            );
            addResult(
              `ℹ️  Please create ${bucketConfig.name} bucket manually in Supabase Dashboard`,
            );
          } else {
      ...
      addResult(
        "✅ Supported file types: images, PDFs, text files, Word documents",
      );
        ...
        <CardContent className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded p-4">
            <p className="text-sm text-blue-800">
              This page checks if your Supabase storage buckets are set up
              correctly for file uploads.
            </p>
          </div>
