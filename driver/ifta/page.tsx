...
export default function DriverIftaLogsPage() {
  ...
  async function save() {
    if (!driverId) {
      alert("No driver account found.");
      return;
    }
    if (!startMiles || !endMiles) {
      alert("Enter start and end miles");
      return;
    }
    ...
    <main className="p-6 max-w-3xl mx-auto space-y-6">
      <header>
        ...
        <p className="text-gray-600 text-sm">
          Enter start/end miles for your day and sign below.
        </p>
      </header>
          ...
          <div className="grid md:grid-cols-4 gap-3 items-end">
            ...
            <div className="text-sm">
              ...
              <div className="w-full border p-2 rounded bg-gray-50">
                {(Number(endMiles || 0) - Number(startMiles || 0)).toFixed(1)}
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Driver Signature</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={save} disabled={saving || !driverId}>
              {saving ? "Saving…" : "Save Log"}
            </Button>
          </div>
          <p className="text-xs text-gray-600">
            By saving, you attest that the entered start and end miles are
            accurate.
          </p>
        </CardContent>
      </Card>
      ...
      <p className="text-sm text-gray-600">
        Need to upload CSVs? Go to Admin → IFTA.
      </p>
    </main>
  );
}
